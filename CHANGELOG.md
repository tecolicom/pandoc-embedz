...
### Fixed
- `_validate_convert_result` now accepts `RawBlock` in addition to `CodeBlock`
  - Allows templates to output raw format blocks (e.g. `` ```{=latex} ``) without triggering a validation error

## [0.22.2] - 2026-03-11

### Fixed
- SSV column splitting now uses only ASCII spaces as separators
  - `str.split()` treated NBSP and full-width space as separators, breaking fields containing these characters
  - Use `re.split(r' +', ...)` to split only on ASCII spaces

## [0.22.1] - 2026-02-15

### Added
- Man page (`man pandoc-embedz`) with comprehensive reference documentation
- `man/pandoc-embedz.md` as source, `man/pandoc-embedz.1` pre-built for distribution
- `data_files` configuration in `pyproject.toml` for man page installation via pip

## [0.22.0] - 2026-02-14

### Added
- `startrow` parameter as replacement for `skiprows`
  - Integer values are 1-indexed (row number to start from), unlike `skiprows` which was 0-indexed (rows to skip)
  - String and list forms behave identically to `skiprows`
  - `skiprows` is now deprecated with a warning; will still work for backward compatibility
  - Specifying both `startrow` and `skiprows` raises an error
...
### Added
- `skiprows` parameter now accepts a list of patterns (AND logic: all must match in the same row)
  - Example: `skiprows: [年, 月]` finds the row containing both "年" and "月"
  - Each element supports existing `"N:text"` format
...
### Fixed
- Strip trailing NaN/None from ragged rows in CSV/SSV with `header=false`
  - pandas 2.x pads short rows with `None`, pandas 3.0 uses `float('nan')`
  - Both are now removed from row tails, preventing "nan" in rendered output
...
[0.22.0]: https://github.com/tecolicom/pandoc-embedz/compare/v0.21.0...v0.22.0
